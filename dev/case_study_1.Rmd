...
# Plot
g <- ggplot(df, aes(region, ABV))
g + geom_boxplot(varwidth=T, fill="plum") + 
    labs(title="Box plot", 
         subtitle="ABV grouped by State REgion",
         caption="Source: beer dataset",
         x="State Region",
         y="ABV")
...
dat2 <- ddply(df, ~ City, transform, IBU = impute.mean(IBU),
     ABV = impute.mean(ABV))
dat2 <- ddply(dat2, ~ State, transform, IBU = impute.mean(IBU),
     ABV = impute.mean(ABV))
dat2 <- ddply(dat2, ~ Style, transform, IBU = impute.mean(IBU),
     ABV = impute.mean(ABV))
...
median_state = dat2%>%
group_by(State)%>% 
summarise_each(funs(median), c("IBU", "ABV"))
head(median_state)
...
ddply(df, .(State), summarize,  Rate1=median(IBU), Rate2=median(ABV))
...
```{r}
ggplot(abv_ibu, aes(x = region, y = IBU)) +
  geom_bar(
    aes(color = region, fill = region),
    stat = "identity", position = position_stack()
    )
...
```{r}
ggplot(abv_ibu) +geom_jitter(aes(x = region, y = IBU))
```
...
rr.summary <- df %>%
  group_by(State, region) %>%
  summarise(
    abv1 = median(ABV, na.rm = TRUE),
    IBU_1 = median(IBU, na.rm = TRUE)
  )
rr.summary
```
...
ggplot(
  abv_ibu, 
  aes(x = region, y = IBU, ymin = IBU-sd, ymax = IBU+sd)
  )
```
...
```{r}
scatter_plot <- ggplot(df, aes(IBU, ABV))
scatter_plot + geom_point() + labs(x = "IBU", y = "ABV")+
      labs(title="Correlation Scatter Plot", 
           subtitle="x-y scatter plot between ABV and IBU", 
           caption="Source: IBU and ABV values are from 'beer and brewery' dataset") 
```
...
```{r}
ggplot(abv_ibu_region, aes(x = region, y = IBU)) +
  geom_bar(
    aes(color = region, fill = region),
    stat = "identity", position = position_stack()
    )
